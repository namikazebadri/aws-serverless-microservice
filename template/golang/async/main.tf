...
resource "aws_sqs_queue" "MODULE_NAME_FUNCTION_NAME_dead_letter_queue" {
...
resource "aws_sqs_queue" "MODULE_NAME_FUNCTION_NAME_queue" {
  name                      = "${local.project}-${var.ENV}_${local.module}_${local.function}"
  delay_seconds             = 0
  max_message_size          = 262144
  message_retention_seconds = 86400
  receive_wait_time_seconds = 10
  redrive_policy            = "{\"deadLetterTargetArn\":\"${aws_sqs_queue.MODULE_NAME_FUNCTION_NAME_dead_letter_queue.arn}\",\"maxReceiveCount\":1}"
}
...
resource "aws_lambda_event_source_mapping" "event_source_mapping" {
  batch_size        = 1
  event_source_arn  = aws_sqs_queue.MODULE_NAME_FUNCTION_NAME_queue.arn
  enabled           = true
  function_name     = aws_lambda_function.MODULE_NAME_FUNCTION_NAME.arn
}