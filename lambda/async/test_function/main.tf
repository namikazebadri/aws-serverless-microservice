...
resource "null_resource" "function_binary" {
  ...
  provisioner "local-exec" {
    command = "export PATH=/opt/homebrew/opt/go/libexec/bin:$PATH && go version && GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GOFLAGS=-trimpath go build -mod=readonly -ldflags='-s -w' -o ${local.binary_path} ${local.src_path}"
  }
}
